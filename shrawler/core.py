...
class Shrawler(SnafflerEngineMixin):
    ...
    def __init__(self) -> None:
        ...
        parser.add_argument(
            "--operating-mode",
            choices=["legacy", "shares", "spider", "snaffle"],
            default="legacy",
        ...
        parser.add_argument(
            "--scan-profile",
            choices=["quiet", "balanced", "fast"],
            default="balanced",
            help="Performance/noise preset (default: balanced)",
        )
        parser.add_argument(
            "--workers",
            type=int,
            help="Number of hosts to scan concurrently (preset default)",
        )
        parser.add_argument(
            "--permission-check",
            choices=["none", "read", "read-write"],
            help="Share permission probing level (preset default)",
        )
        parser.add_argument(
            "--output-mode",
            choices=["summary", "progress", "matches", "tree"],
            help="Terminal output detail (preset default)",
        )
        parser.add_argument(
            "--resume",
            nargs="?",
        ...
        parser.add_argument(
            "--metrics",
            action="store_true",
            help="Display SMB operation counts and timings",
        )
        parser.add_argument(
            "--read-only",
            dest="read_only",
        ...
        parser.add_argument(
            "--output-dir",
            default=".",
            help="Directory for reports and downloaded files (default: current directory)",
        )
        ...
        nemesis.add_argument(
            ...
            default="off",
            help="Nemesis delivery policy (default: off)",
        )
        nemesis.add_argument(
            "--nemesis-upload-workers",
            type=int,
            default=2,
            help="Concurrent Nemesis uploads (default: 2)",
        )
        nemesis.add_argument(
            "--nemesis-retries",
            type=int,
            default=2,
            help="Retries after the initial Nemesis upload attempt (default: 2)",
        )
        nemesis.add_argument(
            "--nemesis-queue-size",
            type=int,
            default=100,
            help="Maximum queued Nemesis uploads before backpressure (default: 100)",
        )
        ...
        spider.add_argument(
            ...
            "You can also specify '--download-ext default' for shrawler to choose the extensions for you.",
        )
        spider.add_argument(
            "--download-name",
            action="store",
        ...
        spider.add_argument(
            "--max-file-size",
            type=int,
            help="Skip downloads larger than this many bytes",
        )
        spider.add_argument(
            "--max-total-download",
            type=int,
            help="Stop downloading after this many total bytes",
        )
        spider.add_argument(
            "--count-ext",
            action="store",
        ...
        spider.add_argument(
            ...
            action="store_true",
            dest="csv_output",
            help="Output results in CSV format (generates shrawler_shares.csv, shrawler_files.csv, shrawler_downloads.csv, and shrawler_snaffler_matches.csv when Snaffler mode is enabled)",
        )
        spider.add_argument(
            "--json-output",
            action="store_true",
        ...
        spider.add_argument(
            "--snaffler-rules-dir",
            action="store",
            dest="snaffler_rules_dir",
            help="Load Snaffler TOML rules recursively from a directory",
        )
        spider.add_argument(
            "--snaffler-interest-level",
            action="store",
            dest="snaffler_interest_level",
            type=int,
            choices=[0, 1, 2, 3],
            help="Minimum Snaffler interest level to include (0-3)",
        )
        spider.add_argument(
            "--snaffler-max-size-to-grep",
            action="store",
            dest="snaffler_max_size_to_grep",
            type=int,
            default=1024 * 1024,
            help="Max file size in bytes for Snaffler content matching (default: 1048576)",
        )
        spider.add_argument(
            "--snaffler-strict",
            action="store_true",
            dest="snaffler_strict",
            help="Fail on malformed or unsupported Snaffler rules",
        )
        spider.add_argument(
            "--snaffler-no-auto-download",
            action="store_true",
            dest="snaffler_no_auto_download",
            help="Do not auto-download files matched by Snaffler rules",
        )
        spider.add_argument(
            "--snaffler-content-mode",
            choices=["relayed", "all"],
            help="Inspect content only through relay rules or for all content rules",
        )
        spider.add_argument(
            "--max-content-reads",
            type=int,
            help="Maximum number of remote files to read for content matching",
        )
        spider.add_argument(
            "--content-read-budget",
            type=int,
            help="Maximum estimated bytes read for content matching",
        )

        self.args = parser.parse_args()
        output_mode_was_explicit = self.args.output_mode is not None
        if self.args.operating_mode == "shares":
            self.args.spider = False
        elif self.args.operating_mode in {"spider", "snaffle"}:
            self.args.spider = True
        if (
            self.args.operating_mode == "snaffle"
            and not self.args.snaffler_rules_dir
        ):
            parser.error("snaffle mode requires --snaffler-rules-dir")
        profile_defaults = {
            "quiet": {
                "workers": 1,
        ...
        }[self.args.scan_profile]
        for option, value in profile_defaults.items():
            if getattr(self.args, option) is None:
                setattr(self.args, option, value)
        if self.args.operating_mode == "shares" and not output_mode_was_explicit:
            self.args.output_mode = "tree"
        if self.args.read_only:
            self.args.permission_check = "read"
        if self.args.host and self.args.hosts_file:
            parser.error("--host and --hosts-file are mutually exclusive")
        ...
        for option in (
            ...
            value = getattr(self.args, option)
            if value is not None and value < 0:
                parser.error(f"--{option.replace('_', '-')} must be zero or greater")
        if self.args.workers < 1:
            parser.error("--workers must be one or greater")
        if self.args.nemesis_upload_workers < 1:
            parser.error("--nemesis-upload-workers must be one or greater")
        if self.args.nemesis_retries < 0:
            parser.error("--nemesis-retries must be zero or greater")
        if self.args.nemesis_queue_size < 1:
            parser.error("--nemesis-queue-size must be one or greater")
        if self.args.nemesis_mode != "off":
            missing_nemesis = [
                name
                for name, value in (
                    ("--nemesis-url", self.args.nemesis_url),
                    ("--nemesis-auth", self.args.nemesis_auth),
                    ("--nemesis-project", self.args.nemesis_project),
                )
                if not value
            ]
            if missing_nemesis:
                parser.error(
                    "Nemesis mode requires " + ", ".join(missing_nemesis)
                )
            if ":" not in self.args.nemesis_auth:
                parser.error("--nemesis-auth must use username:password format")
        if (
            self.args.operating_mode == "spider"
            and self.args.nemesis_mode == "matches"
        ):
            parser.error("spider mode supports Nemesis modes 'off' and 'downloads'")
