...
class CollectionQueue:
    def __init__(self, database):
        ...
        with self.connect() as db:
            db.execute('CREATE TABLE IF NOT EXISTS manifests (id TEXT PRIMARY KEY, payload TEXT NOT NULL)')
    ...
    def get(self, identifier):
        with self.connect() as db:
            row = db.execute('SELECT payload FROM manifests WHERE id=?', (identifier,)).fetchone()
        if row is None:
    ...
    def list(self):
        with self.connect() as db:
            return [json.loads(row[0]) for row in db.execute('SELECT payload FROM manifests ORDER BY rowid DESC')]

    def create(self, *, run_id=None, category=None, limit=100, min_score=0,
               max_file_size=50 * 1024**2, max_total_bytes=500 * 1024**2,
               file_ids=None, name='Collection'):
        ...
        previously = {item['file_id'] for manifest in self.list()
                      for item in manifest['items'] if item['status'] == 'collected'}
        ...
        try:
            ...
            prior_paths = {(h.casefold(), s.casefold(), self._path(p)) for h, s, p in rows if p}
        finally:
            source.close()
        items = []
        expected = 0
        for candidate in candidates['items']:
            if file_ids is not None and candidate['file_id'] not in file_ids:
                continue
            prior = candidate['file_id'] in previously or (
                candidate['host'].casefold(), candidate['share'].casefold(),
                self._path(candidate['remote_path'])) in prior_paths
            ...
            if not prior:
                if size > max_file_size or expected + size > max_total_bytes:
                    ...
                    expected += size
            items.append({**candidate, 'reasons': [s['description'] for s in candidate['signals'] if s['credited_points'] > 0],
                          'previously_collected': prior, 'status': status, 'attempts': [], 'local_path': None})
        manifest = {'version': 1, 'id': uuid.uuid4().hex, 'name': name,
                    'created_at': utc_now(), 'source_database': str(self.database),
                    'query': {'run_id': candidates['run_id'], 'category': category, 'limit': limit,
                              'min_score': min_score, 'file_ids': file_ids},
                    'scan_id': candidates['scan_id'], 'max_file_size': max_file_size,
                    'max_total_bytes': max_total_bytes, 'consumed_bytes': 0,
                    'expected_files': sum(i['status'] == 'pending' for i in items),
                    'expected_bytes': expected, 'items': items}
        self._save(manifest)
        return manifest

    @staticmethod
    def _path(value):
        return '/' + '/'.join(p for p in value.replace('\\', '/').casefold().split('/') if p)

    def run(self, identifier, retrieve):
        """One attempt per unfinished item. Sink accounting includes failed reads."""
        with self.path.open('rb') as lock:
            try:
                fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
            except BlockingIOError:
                raise CollectionBusyError('A collection is already running for this inventory')
            ...
            for item in manifest['items']:
                ...
                if item['status'] == 'collecting' and item['attempts']:
                    item['attempts'][-1].update(status='interrupted', finished_at=utc_now())
                remaining = manifest['max_total_bytes'] - manifest['consumed_bytes']
                if remaining < item['size_bytes']:
                    item['status'] = 'failed'
                    item['error'] = 'Remaining total budget is smaller than expected file size'
                    self._save(manifest)
                    continue
                ...
                try:
                    with path.open('xb') as handle:
                        ...
                        def sink(data):
                            ...
                            if (attempt['bytes'] + len(data) > manifest['max_file_size'] or
                                    manifest['consumed_bytes'] + len(data) > manifest['max_total_bytes']):
                                raise ValueError('Actual content exceeds collection byte limit')
...
def main(argv=None):
    ...
    from .cli import _create_auth, add_smb_auth_arguments
    from .config import parse_size
    from .output import escape_terminal

    parser = argparse.ArgumentParser(prog='shrawler collect', description='Review and retrieve saved collection manifests without scanning.')
    ...
    run = commands.add_parser('run', help='retrieve pending or failed items; repeat to resume')
    ...
    try:
        ...
        if args.command == 'create':
            ...
            result = queue.run(args.id, smb_retriever(_create_auth(args, args.auth)))
        print(json.dumps(result, indent=2, ensure_ascii=True))
        if args.command == 'run' and any(i['status'] == 'failed' for i in result['items']):
            return 1
        return 0
